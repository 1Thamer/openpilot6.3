      ...
      if socket is pathPlan:
        ...
        for _pp in _pathPlan:
          ...
          if vEgo > 0 and active and (carState == None or boolStockRcvd):
            ...
            pathDataString +=  (pathDataFormatString % (pp.lProb, pp.rProb, pp.laneWidth, pp.rateSteers, int((monoTimeOffset + _pp.logMonoTime) * .0000002) * 5))
        ...
        for l100 in _controlsState:
          if lateral_type == "":
            if l100.controlsState.lateralControlState.which == "pidState":
              lateral_type = "pid"
              influxFormatString = user_id + ",sources=capnp ff_angle=1,angle_steers_des=%s,angle_steers=%s,steer_override=%s,v_ego=%s,p=%s,i=%s,f=%s,output=%s %s\n"
          ...
          if vEgo > 0 and active:
            ...
            if lateral_type == "pid":
              influxDataString += ("%0.3f,%0.4f,%d,%0.1f,%0.4f,%0.4f,%0.4f,%0.4f,%d|" %
